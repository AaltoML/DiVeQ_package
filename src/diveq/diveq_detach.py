...
class DIVEQDetach(nn.Module):
    """
    DiVeQ_Detach: Detach version of Differentiable Vector Quantization (VQ) module that
    allows end-to-end training of VQ-based models without any auxiliary losses or
    hyperparameter tunings. The module encompasses codebook replacement function which
    discards unused codebook entries during training.
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, float]:
        ...
        vq_error = error_magnitude * ((z_hard_quantized - z) / (error_magnitude
                                                                    + 1e-12)).detach()
    ...
    def _check_input(self, z: torch.Tensor) -> None:
        ...
        if z.size(1) != self.embedding_dim:
            raise ValueError(f"DiVeQ_Detach input.shape[1] must match the embedding"
